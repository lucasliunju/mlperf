...
def model_fn(features, labels, mode, params, tf_sess=False):
    ...
    policy_output, value_output, logits = model_inference_fn(
        features, mode == tf.estimator.ModeKeys.TRAIN, params) # logits 是什么， 模型走一次

    # train ops
    policy_cost = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits_v2(
            logits=logits, labels=tf.stop_gradient(labels['pi_tensor']))) # 策略是one-hot的形式，记录策略的损失值

    value_cost = params['value_cost_weight'] * tf.reduce_mean(
        tf.square(value_output - labels['value_tensor'])) # 价值函数的损失

    reg_vars = [v for v in tf.trainable_variables()
                if 'bias' not in v.name and 'beta' not in v.name] # 可训练的参数
    l2_cost = params['l2_strength'] * \
        tf.add_n([tf.nn.l2_loss(v) for v in reg_vars]) # l2范数的损失

    combined_cost = policy_cost + value_cost + l2_cost # 总的

    global_step = tf.train.get_or_create_global_step()
    learning_rate = tf.train.piecewise_constant(
        global_step, params['lr_boundaries'], params['lr_rates'])
    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS) # 从一个集合中取出变量
    ...
    if params['quantize']:
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            contrib_quantize.create_eval_graph() # 模型量化

    optimizer = tf.train.MomentumOptimizer(
        learning_rate, params['sgd_momentum']) # 动量优化器

    # hvd multigpu
    optimizer = hvd.DistributedOptimizer(optimizer) # 多GPU分布式优化

    if params['use_tpu']:
        optimizer = contrib_tpu_python_tpu_tpu_optimizer.CrossShardOptimizer(
            optimizer)
    with tf.control_dependencies(update_ops):
        train_op = optimizer.minimize(combined_cost, global_step=global_step) # 根据损失进行模型的参数优化
...
def model_inference_fn(features, training, params):
    ...
    def mg_activation(inputs):
        if FLAGS.use_swish: # 是否使用新的激活函数
            return tf.nn.swish(inputs)

        return tf.nn.relu(inputs)

    def residual_inner(inputs): # 定义一个网络结构
    ...
    def mg_res_layer(inputs): # 进一步定义网络结构
        residual = residual_inner(inputs)
        output = mg_activation(inputs + residual)
        return output

    def mg_squeeze_excitation_layer(inputs): # 定义了一种更新的网络结构
    ...
    initial_block = mg_activation(mg_batchn(mg_conv2d(features))) # 定义一个网络结构

    # the shared stack
    shared_output = initial_block
    for _ in range(params['trunk_layers']): # 选择一种具体的网络结构
    ...
    policy_output = tf.nn.softmax(logits, name='policy_output') # 所有可能位置的概率
    ...
    value_output = tf.nn.tanh(
        tf.reshape(tf.layers.dense(value_fc_hidden, 1), [-1]), # 输出一个具体的值
        name='value_output')
