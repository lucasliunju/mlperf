...
from mpi4py import MPI #并行通信的包包
import socket
...
def main(unused_argv):
    ...
    # MPI
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank() #获得当前的进程号
    size = comm.Get_size() #获得当前的进程数
    node = rank // FLAGS.ranks_per_node #获取进程属于哪一个节点
    ...
    selfplay_ranks, selfplay_data_transfer_ranks = distribute_mpiranks(rank, size) #分配执行不同任务的进程
    ...
    if rank in train_ranks:
        tstate = minigo_train.init_train(rank, tcomm, FLAGS.model_dir) #在所有的训练进程中初始化
